...
function inferCanonicalIntentFromEvalContext(value, item = {}) {
  const combined = [
    value,
    item?.question,
    item?.expectedSource,
    item?.expectedAnswerHint,
    item?.adminNote,
  ...
  const primaryCompact = compactText(primaryText);
  const has = (pattern) => pattern.test(combined) || pattern.test(normalized) || pattern.test(compact);
  const hasPrimary = (pattern) => pattern.test(primaryText) || pattern.test(primaryNormalized) || pattern.test(primaryCompact);

  // Specific intents must be checked before broad topics such as reservation or surgery.
  if (has(/수술\s*후.{0,12}(출혈|피|피가|피나|피남)|수술후.{0,12}(출혈|피|피가|피나|피남)|수술후출혈|편도.{0,10}출혈/u)) {
    return 'postop_bleeding';
  }

  if (
    has(/입원전복용중단약물|복용중단|중단약물|금지약물|수술전.{0,12}(복용약|약물|약)|입원전.{0,12}(복용약|약물|약)|복용중인약|아스피린|항응고|항혈소판/u)
  ) {
    return 'medication_stop';
  }

  if (has(/수술후코세척|수술후.{0,8}코세척/u)) {
    return 'nasal_irrigation_surgery';
  }

  if (has(/일반코세척|일반.{0,8}코세척/u)) {
  ...
  if (has(/검사및주사금액|검사금액|검사비용|주사금액|주사비용|수액.{0,8}(비용|금액)|알레르기주사.{0,8}(비용|금액)/u)) {
    return 'exam_injection_fee';
  }

  if (has(/후각.{0,8}(검사|저하|장애)|후각장애|냄새.{0,8}(검사|안남|못맡)/u)) {
    return 'smell_exam';
  }

  if (has(/층별안내|층별안내도|몇\s*층|원무과.{0,8}(위치|어디)|검사실.{0,8}(위치|어디)|청력검사실|주사실|화장실|약국/u)) {
    return 'floor_guide';
  }

  if (has(/보청기|청각보조기/u)) {
    return 'hearing_aid_consult';
  }

  if (has(/청력검사|난청.{0,8}검사|청각검사/u)) {
    return 'hearing_test_process';
  }

  if (has(/수면검사|수면다원검사|수면무호흡|코골이검사/u)) {
    return 'sleep_apnea_exam';
  }

  if (has(/서류발급|진료확인서|진단서|소견서|진료기록|검사결과지|결과지|세부내역서|영수증|사본|의무기록/u)) {
    return 'receipt_issuance';
  }

  if (has(/서류비용|진단서비용|증명서비용|제증명|발급비용/u)) {
    return 'certificate_fee';
  }

  if (has(/주차|발렛|주차장|주차비|주차권/u)) {
    return 'parking_info';
  }

  if (hasPrimary(/예약변경|예약취소|예약방법|예약문의|진료예약|외래예약|예약하고싶|예약하려|예약/u)) {
    return 'reservation_or_reception';
  }

  if (has(/입원준비물|입원물품|입원전준비|입원.{0,8}챙/u)) {
    return 'admission_prep_items';
  }

  if (has(/보호자식|보호자식사|보호자밥/u)) {
    return 'guardian_meal';
  }

  if (has(/의료진|의사|원장|전문분야|센터의료진|담당의/u)) {
    return has(/경력|약력|이력/u) ? 'doctor_career' : 'doctor_overview';
  }

  if (hasPrimary(/전화번호|대표전화|병원전화/u)) {
    return 'hospital_phone';
  }

  if (has(/지각|늦음|늦을|예약시간/u)) {
    return 'late_arrival';
  }

  if (has(/당일검사|검사당일|당일.{0,8}검사|오늘.{0,8}검사/u)) {
    return 'same_day_exam_availability';
  }

  if (has(/편도수술|편도절제|편도/u)) {
    return 'tonsillectomy_info';
  }

  if (has(/진료의뢰|의뢰서|타병원자료|타병원진료/u)) {
    return 'referral_document';
  }

  if (has(/진료시간|야간진료|주말진료|토요일진료|일요일|공휴일/u)) {
    return 'clinic_hours_night_weekend';
  }

  if (has(/목멍울|목의혹|갑상선혹|목에멍울/u)) {
    return 'throat_mass_care';
  }

  if (has(/귀먹먹|먹먹함|이명|귀에서소리|소리가잘안/u)) {
    return 'ear_fullness_hearing_loss';
  }
...
function normalizeExpectedIntent(value, item = {}) {
  const raw = String(value || '').trim();
  if (!raw) {
    return inferCanonicalIntentFromEvalContext('', item);
  }
  ...
  if (hasAny(['예약늦음', '예약시간늦음', '예약지각', '예약시간지각', '늦을때', '늦게도착'])) {
    return 'late_arrival';
  }
  ...
  if (hasAny(['검사금액', '검사비용', '검사비', '주사금액', '주사비용', '주사비', '검사및주사금액', '검사주사금액'])) {
    return 'exam_injection_fee';
  }
  ...
  if (hasAny(['청력검사', '청력검사방법', '청력검사진행', '난청검사'])) {
    return 'hearing_test_process';
  }
  if (hasAny(['수면검사', '수면다원검사', '수면무호흡검사', '코골이검사'])) {
    return 'sleep_apnea_exam';
  }
  if (hasAny(['편도수술', '편도절제술', '편도절제수술', '편도'])) {
    return 'tonsillectomy_info';
  }
  if (hasAny(['진료의뢰', '진료의뢰서', '의뢰서', '타병원자료'])) {
    return 'referral_document';
  }
  if (hasAny(['당일진료', '진료시간', '야간진료', '주말진료', '토요일진료'])) {
    return 'clinic_hours_night_weekend';
  }
  if (hasAny(['경력', '의료진경력', '의사경력'])) {
    return 'doctor_career';
  }
  if (hasAny(['성대물혹', '목혹', '목멍울', '갑상선혹'])) {
    return 'throat_mass_care';
  }
  if (hasAny(['귀먹먹', '귀먹먹함', '이명', '난청', '소리가안들림'])) {
    return 'ear_fullness_hearing_loss';
  }
  if (hasAny(['층별안내', '층별안내도', '층안내', '1층', '2층', '3층', '4층', '5층', '6층', '7층', '지하1층', '지하2층'])) {
    return 'floor_guide';
  }

  return inferCanonicalIntentFromEvalContext(raw, item) || 'eval_source_guidance';
}
...
function getEvalIntentIndex() {
  ...
  EVAL_FILES.forEach((filePath) => {
    readJsonArray(filePath).forEach((item) => {
      ...
      const expectedIntent = normalizeExpectedIntent(item?.expectedIntent || '', item);
